...
"""Per-session consult and health markers for the Codex advisor guardrail."""
...
MARKER_DIR_NAMES = ("codex-as-advisor-guardrail-markers", "advisor-guardrail-markers")
HEALTH_SKILL = "codex-advisor-health"
...
def health_path(session_id: str) -> Path:
    return marker_dir() / f"advisor-health-{session_id}.json"
...
def offline_reason(session_id: str) -> str:
    ...
    return reason if isinstance(reason, str) and reason.strip() else "advisor unavailable"
...
def presence_line(session_id: str | None = None) -> str:
    if not session_id:
        return "Codex-as-advisor status unknown."
    ...
    if state == "online":
        return (
            f"Codex-as-advisor online — model {model}, effort {effort}, fast {fast_text}."
        )
    if state == "offline":
        reason = offline_reason(session_id)
        return (
            f"Codex-as-advisor offline — {reason}. Write gate disabled for this session. "
            f"Retest with the {HEALTH_SKILL} skill."
        )
    return (
        f"Codex-as-advisor health pending — model {model}, effort {effort}, fast {fast_text}. "
