# codex-as-critic-guardrail (Claude Code host)

Cross-vendor actor-critic guardrail for **Claude Code**. The executor must
consult an antagonistic, read-only critic at decision points, and the
session's first write is gated while the critic is healthy and no consult has
succeeded yet. The critic backend is the local **Codex CLI** (not the Codex
IDE). Same-family models share blind spots; the critic exists to catch what
the executor's family would miss alone.

The Cursor host of this same guardrail is a separate copy under
`plugins/cursor/codex-as-critic-guardrail/`; the two share no code or
manifest.

| How to consult | Unlock | MCP? |
| --- | --- | --- |
| Shell → `cli/consult_critic.py` (stdin JSON) | `PostToolUse` matcher `Bash` | **No** |
