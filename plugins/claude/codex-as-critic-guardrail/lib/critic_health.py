...
"""Bounded health probe for the Codex critic backend."""
...
from critic_consult import run_codex_prompt
from critic_session import (
    clear_consulted,
    format_status_block,
    mark_offline,
    mark_online,
...
HEALTH_PROMPT = (
    "Reply with exactly the six characters CRITIC_OK and nothing else. "
...
def run_health_probe(
    ...
    *,
    ...
    if os.environ.get("CODEX_CRITIC_SKIP_HEALTH") == "1":
        mark_online(
            session_id,
            model=loaded.model,
        ...
        return HealthResult(
            ...
            effort=loaded.effort,
            fast=loaded.fast,
            reason="health probe skipped (CODEX_CRITIC_SKIP_HEALTH=1)",
        )
    ...
    try:
        _codex_reachable()
        # require_critic_config raises only on invalid harness; defaults are fine.
        require_critic_config(workspace)
