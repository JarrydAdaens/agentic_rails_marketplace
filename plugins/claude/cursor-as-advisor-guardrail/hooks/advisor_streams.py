...
"""Shared stdio helpers for the cursor-as-advisor-guardrail hooks.
...
def read_hook_payload() -> dict[str, Any] | None:
    ...
    try:
        ...
        print(
            f"advisor hook could not read stdin ({type(exc).__name__}: {exc}); allowing.",
            file=sys.stderr,
        )
        return None
    ...
    try:
        ...
        print(
            f"advisor hook received invalid JSON on stdin ({exc}); "
    ...
    if not isinstance(payload, dict):
        print(
            f"advisor hook received a {type(payload).__name__} on stdin, expected an object; allowing.",
            file=sys.stderr,
        )
        return None
