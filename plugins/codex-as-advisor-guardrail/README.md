...
Cross-vendor actor-advisor guardrail for **Claude Code** and **Cursor**. The
executor must consult a constructive, read-only advisor at decision points, and
the session's first write is gated while the advisor is healthy and no consult
has succeeded yet. The advisor backend is the local **Codex CLI** (not the Codex
...
| --- | --- | --- | --- |
| Claude Code | MCP `consult_advisor` | PostToolUse on that tool | Yes (`.mcp.json`) |
| Cursor | Shell → `cli/consult_advisor.py` (stdin JSON) | `afterShellExecution` | **No** |
