...
"""Record a successful advisor consult (MCP PostToolUse or Cursor afterShellExecution)."""
...
def _is_mcp_consult(payload: dict) -> bool:
    ...
    return tool_name == "consult_advisor" or tool_name.endswith("consult_advisor")
...
def _is_cli_consult(payload: dict) -> bool:
    ...
    if "Codex advisor" in output and ("failed" in output.lower() or "timed out" in output.lower()):
        return False
    return True
