...
class HookTests(unittest.TestCase):
    ...
    def test_gate_uses_hook_specific_output_shape(self):
        denied = json.loads(self.invoke(advisor_gate, {"session_id": "c", "tool_name": "Edit"}))
        self.assertEqual(denied["hookSpecificOutput"]["permissionDecision"], "deny")
        with patch.object(advisor_gate, "has_marker", return_value=True):
            self.assertEqual(self.invoke(advisor_gate, {"session_id": "c", "tool_name": "Write"}), "")
    ...
    def test_context_uses_claude_plain_text_shape(self):
