...
correction, or a completion verdict. Consults are cheap relative to mistakes but
expensive relative to silence — target 2–3 per task.
...
`claude-advisor-health` skill when auth, quota, model, or config is fixed. If
...
health is online and no advisor consult has succeeded. The deny message tells
