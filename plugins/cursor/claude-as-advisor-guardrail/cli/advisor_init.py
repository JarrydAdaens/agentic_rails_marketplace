...
"""Write the default harness config for claude-as-advisor-guardrail."""
...
from advisor_config import CONFIG_RELATIVE_PATH, write_default_config  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Write harness/claude-as-advisor-guardrail/cursor-config.json with defaults and comments."
    )
    parser.add_argument(
        "--workspace",
    ...
    print("Claude-as-advisor config initialized")
    print(f"Path: {path}")
