...
"""Project harness config for the Claude advisor (model / effort / timeouts)."""
...
CONSULT_TIMEOUT_ENV_VAR = "CLAUDE_ADVISOR_TIMEOUT_SECONDS"
HEALTH_TIMEOUT_ENV_VAR = "CLAUDE_ADVISOR_HEALTH_TIMEOUT_SECONDS"

# The levels `claude --effort` accepts. The Claude CLI has no fast-tier flag,
# so this plugin's config carries no `fast` field.
EFFORTS = ("low", "medium", "high", "xhigh", "max")

# Written by claude-advisor-init. JSONC: // comments allowed; the loader strips them.
DEFAULT_CONFIG_TEMPLATE = """\
{
  ...
  // Wall-clock seconds for a full advisor consult before hard kill.
  // Override with env CLAUDE_ADVISOR_TIMEOUT_SECONDS when set.
  "consult_timeout_seconds": 600,

  // Wall-clock seconds for the session-start / skill health probe.
  // Override with env CLAUDE_ADVISOR_HEALTH_TIMEOUT_SECONDS when set.
