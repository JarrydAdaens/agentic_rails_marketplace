---
name: claude-advisor-timeout
description: >-
  View or persist the Claude advisor consult timeout for this project. Use when
  the user asks how long advisory consults may run, wants to change that limit,
  restore its default, or cancel a pending change.
argument-hint: "[seconds|default|cancel]"
disable-model-invocation: true
---

# Claude advisor consult timeout
...
```text
uv run --no-project python ./scripts/launch.py ./cli/advisor_timeout.py --seconds --workspace <PROJECT_ROOT>
```

With a user value, run:

```text
uv run --no-project python ./scripts/launch.py ./cli/advisor_timeout.py --seconds <USER_VALUE> --workspace <PROJECT_ROOT>
```
