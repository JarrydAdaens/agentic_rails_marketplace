...
class HookTests(unittest.TestCase):
    ...
    def test_gate_allows_when_pending_or_offline(self):
        with patch.object(advisor_gate, "has_marker", return_value=False), patch.object(advisor_gate, "health_state", return_value="pending"):
            pending = json.loads(self.invoke(advisor_gate, GATE_PAYLOAD))
        ...
        with patch.object(advisor_gate, "has_marker", return_value=False), patch.object(advisor_gate, "health_state", return_value="offline"), patch.object(advisor_gate, "offline_reason", return_value="no credits"):
            offline = json.loads(self.invoke(advisor_gate, GATE_PAYLOAD))
    ...
    def test_gate_denies_when_online_and_unconsulted(self):
        with patch.object(advisor_gate, "has_marker", return_value=False), patch.object(advisor_gate, "health_state", return_value="online"):
            denied = json.loads(self.invoke(advisor_gate, GATE_PAYLOAD))
    ...
    def test_gate_still_denies_a_bom_prefixed_payload(self):
        ...
        with patch.object(advisor_gate, "has_marker", return_value=False), patch.object(advisor_gate, "health_state", return_value="online"):
            denied = json.loads(self.invoke(advisor_gate, None, raw="﻿" + json.dumps(GATE_PAYLOAD)))
        self.assertEqual(denied["permission"], "deny")
    ...
    def test_failed_or_unrelated_commands_do_not_mark(self):
        with patch.object(advisor_marker, "mark_consulted") as consulted:
            self.invoke(advisor_marker, {"session_id": "no", "command": "git status", "exit_code": 0})
            self.invoke(advisor_marker, {
                "session_id": "no",
                "command": "./cli/consult_advisor.py",
                "exit_code": 1,
            })
        consulted.assert_not_called()

    def test_marker_paths_are_plugin_specific(self):
        self.assertEqual(advisor_markers.marker_dir().name, "claude-as-advisor-guardrail-markers")
    ...
    def test_health_markers_round_trip(self):
        with tempfile.TemporaryDirectory() as temporary, patch.object(advisor_session, "marker_dir", return_value=Path(temporary)):
            ...
            advisor_session.mark_online("s", model="opus", effort="high")
            self.assertEqual(advisor_session.health_state("s"), "online")
            advisor_session.mark_offline("s", "quota", model="opus", effort="high")
...
class ConfigTests(unittest.TestCase):
    def test_missing_config_falls_back_to_defaults(self):
        with tempfile.TemporaryDirectory() as root:
            config = advisor_config.load_advisor_config(root)
    ...
    def test_jsonc_comments_are_stripped_and_fields_validated(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            self.assertEqual(advisor_config.load_advisor_config(root).source, "harness")

            path.write_text('{"effort": "turbo"}', encoding="utf-8")
            broken = advisor_config.load_advisor_config(root)
            self.assertEqual(broken.source, "error")
            self.assertIn("effort", broken.error)
            with self.assertRaises(RuntimeError):
                advisor_config.require_advisor_config(root)
...
class SurfaceTests(unittest.TestCase):
    ...
    def test_skills_are_user_invoked_and_name_their_cli(self):
        health = (PLUGIN / "skills" / "claude-advisor-health" / "SKILL.md").read_text(encoding="utf-8")
        init = (PLUGIN / "skills" / "claude-advisor-init" / "SKILL.md").read_text(encoding="utf-8")
        help_text = (PLUGIN / "skills" / "claude-advisor-help" / "SKILL.md").read_text(encoding="utf-8")
        for skill in (health, init, help_text):
            self.assertIn("disable-model-invocation: true", skill)
        self.assertIn("Result: ONLINE | OFFLINE", health)
        self.assertIn("advisor_health.py", health)
        self.assertIn("advisor_init.py", init)
        for hook in ("preToolUse", "afterShellExecution", "sessionStart"):
            self.assertIn(hook, help_text)

    def test_advisor_init_writes_a_config_and_refuses_to_clobber_it(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            config = Path(root) / "harness" / "claude-as-advisor-guardrail" / "cursor-config.json"
    ...
    def test_advisor_health_cli_reports_online_when_the_probe_is_skipped(self):
        health_path = PLUGIN / "cli" / "advisor_health.py"
        with tempfile.TemporaryDirectory() as root:
            completed = subprocess.run(
                [sys.executable, str(health_path), "--session-id", "probe-test", "--workspace", root],
                env={**os.environ, "CLAUDE_ADVISOR_SKIP_HEALTH": "1"},
                capture_output=True, text=True, timeout=60, check=False,
            )
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertIn("Claude-as-advisor health check", completed.stdout)
    ...
    def test_session_start_injects_the_protocol_as_utf8(self):
        ...
        self.assertIn("Claude Advisor Protocol", emitted)
