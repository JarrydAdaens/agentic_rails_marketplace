...
CLI = PLUGIN / "cli" / "critic_install_hooks.py"
...
SIBLING = {
    ...
    "command": r'uv run --no-project python "C:\Users\Jarry\.cursor\plugins\local\claude-as-advisor-guardrail\scripts\launch.py" "C:\Users\Jarry\.cursor\plugins\local\claude-as-advisor-guardrail\hooks\advisor_gate.py"',
    "failClosed": False,
}
SEED = {
    "version": 1,
...
class UserHookInstallTests(unittest.TestCase):
    def test_install_twice_then_remove_restores_seed(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertIn("Claude-as-critic hooks installed", first.stdout)
            ...
            self.assertTrue(any("claude-as-critic-guardrail" in str(item["command"]) for item in pre))
            self.assertTrue(any("critic_gate.py" in str(item["command"]) for item in pre))

            removed = run_cli(REMOVE, hooks_file)
            self.assertEqual(removed.returncode, 0, removed.stderr)
            self.assertIn("Claude-as-critic hooks removed", removed.stdout)
