...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, RuntimeError) as exc: print(f"Could not remove advisor hooks: {exc}", file=sys.stderr); return 1
    print(f"Codex-as-advisor hooks removed ({removed} entries)\nPath: {path}\nStart a new CLI session for the change to take effect."); return 0
