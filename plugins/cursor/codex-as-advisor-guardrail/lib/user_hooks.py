...
def _owns_entry(entry: Any, plugin_root: Path) -> bool:
    ...
    return bool(root and root in command) or (f"/{plugin_root.name.casefold()}/" in f"/{command}/" and "launch.py" in command)
def _rewrite_entry(entry: dict[str, Any], plugin_root: Path) -> dict[str, Any]:
    ...
    if not launch.is_file(): raise RuntimeError(f"missing launcher: {launch}")
    parts=[]
    for token in command.split():
        normalized=token.replace("\\", "/")
        if normalized in {"./scripts/launch.py", "scripts/launch.py"}: parts.append(_quote(launch))
        elif normalized.startswith("./hooks/") or normalized.startswith("hooks/"):
