---
name: codex-advisor-health
description: >-
  Retest the Codex-as-advisor guardrail health for the current session and report
...
1. Resolve the plugin root for `codex-as-advisor-guardrail` (installed plugin
...
`harness/codex-as-advisor-guardrail/cursor-config.json` when present.
...
succeeds (Claude: `consult_advisor` MCP; Cursor: stdin JSON to
`cli/consult_advisor.py`).
