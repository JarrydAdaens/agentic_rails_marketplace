...
from critic_config import config_path  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Retest Codex-as-critic health for a session.")
    parser.add_argument(
        "--session-id",
    ...
    parser.add_argument(
        ...
        help="Project root containing harness/codex-as-critic-guardrail/cursor-config.json",
    )
    ...
    result = run_health_probe(
        ...
        mark_pending_first=True,
    )
    print(result.status_block())
    path = config_path(args.workspace)
    print(f"Config file: {'FOUND' if path.is_file() else 'MISSING'}")
    print(f"Config path: {path}")
    print("Manual fields: enabled, model, effort, fast, consult_timeout_seconds, health_timeout_seconds")
