...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, RuntimeError) as exc: print(f"Could not install critic hooks: {exc}", file=sys.stderr); return 1
    print(f"Codex-as-critic hooks installed ({added} entries)\nPath: {path}\nStart a new CLI session for the write gate to load."); return 0
