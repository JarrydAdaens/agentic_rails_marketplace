...
try {
    ...
    # UTF-8 BOM arrives as replacement characters rather than U+FEFF. Read the
    # raw stream and decode it as UTF-8 explicitly; StreamReader's BOM detection
    # consumes a leading BOM. Report anything still unparsable rather than
    # degrading to an empty payload in silence.
    $stdinReader = [System.IO.StreamReader]::new(
        [Console]::OpenStandardInput(),
        [System.Text.UTF8Encoding]::new($false),
        $true)
    try { $stdin = $stdinReader.ReadToEnd() } finally { $stdinReader.Dispose() }

    $hook = [pscustomobject]@{}
    if (-not [string]::IsNullOrWhiteSpace($stdin)) {
        $stdin = $stdin.TrimStart([char]0xFEFF)
        try { $hook = $stdin | ConvertFrom-Json }
        catch {
            [Console]::Error.WriteLine("jobs-done-guardrail could not parse its stdin payload ($($_.Exception.Message)); continuing with an empty payload.")
            $hook = [pscustomobject]@{}
        }
    }
