...
---
# Local advisor help
Start with `python ./cli/advisor_version.py` and include its result. This Cursor-only plugin uses bundled native custom subagents, never MCP and never a second Cursor CLI process. The first write is denied until the configured `local-advisor-*` Task/Agent subagent completes. Available controls are `local-advisor-enabled`, `local-advisor-health`, `local-advisor-init`, `local-advisor-model`, `local-advisor-timeout`, and `local-advisor-version`. Model choices are Auto, Cursor Grok 4.6, Composer 2.5, Gemini 3.7 Flash, GPT-5.4-Nano, and Kimi-K3; Cursor owns effort defaults for each model.
