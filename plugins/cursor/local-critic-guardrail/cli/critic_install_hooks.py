"""Install local critic hooks into the user-level Cursor hooks.json."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, RuntimeError) as exc: print(f"Could not install local critic hooks: {exc}", file=sys.stderr); return 1
    print(f"Local-critic hooks installed ({added} entries)\nPath: {path}\nStart a new Cursor CLI session for the write gate to load."); return 0
