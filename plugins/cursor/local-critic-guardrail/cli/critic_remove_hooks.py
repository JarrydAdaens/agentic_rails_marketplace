"""Remove local critic hooks from the user-level Cursor hooks.json."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, RuntimeError) as exc: print(f"Could not remove local critic hooks: {exc}", file=sys.stderr); return 1
    print(f"Local-critic hooks removed ({removed} entries)\nPath: {path}\nStart a new Cursor CLI session for the change to take effect."); return 0
