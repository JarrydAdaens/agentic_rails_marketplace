 ...
 * session — if the Claude CLI cannot be resolved, or a consult times out, the
 * gate DISARMS. A consult that is reachable but fails (non-zero exit) leaves
 * the gate armed: the model sees the failure and can retry.
 *
 ...
 * - unreachable or timedout -> `disarmed` (writes allowed forever: an
 *   unreachable advisor must never wedge the session);
 * - failed     -> unchanged. A reachable-but-failing advisor must not
 *   silently open the gate; the model sees the failure and can retry.
 */
...
export function failedAdvisorMessage(stderr: string, note: string | undefined): string {
	...
	return `Claude advisor failed.${detail} The write gate stays armed: fix the cause and consult again.`;
}
