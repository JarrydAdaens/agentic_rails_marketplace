 ...
 *
 * A constructive, cross-vendor senior engineering advisor: an ephemeral,
 * read-only Codex session consulted before substantive implementation, when
 ...
 * `consult_codex_advisor`, as the MCP replacement. The write gate just
 ...
 * session — if the Codex CLI cannot be resolved, a consult times out, or a
 ...
 * DISARMS. A SOFT failure (transient, malformed, one-off) leaves the gate
 ...
 * `--ephemeral` is what keeps the consult out of Codex's session history;
 * both are carried over. The reply is capped via budget.ts before it
...
export const GUARDRAIL_NAME = "codex-as-advisor-guardrail";

// --- config ------------------------------------------------------------------

/** Config shape: `harness/codex-as-advisor-guardrail/config.json`. */
export interface CodexAdvisorConfig {
	enabled?: boolean;
	/** Model id passed to `codex exec --model`. Default `gpt-5.6-sol`. */
	model?: string;
	/** Reasoning effort. Default `high`. One of {@link VALID_EFFORTS}. */
	effort?: string;
	/** Consult timeout in seconds. Default 1800. */
...
/** Default Codex model, matching the Cursor host. */
export const DEFAULT_MODEL = "gpt-5.6-sol";
/** Valid reasoning efforts, matching the Cursor host. */
export const VALID_EFFORTS = ["minimal", "low", "medium", "high", "xhigh"] as const;
/** Default reasoning effort, matching the Cursor host. */
export const DEFAULT_EFFORT = "high";
/** Default consult timeout: 1800 seconds (the Cursor host's Codex default). */
...
export function configModel(config: CodexAdvisorConfig | null): string {
	const value = config?.model;
	return typeof value === "string" && value.trim() !== "" ? value.trim() : DEFAULT_MODEL;
}

export function configEffort(config: CodexAdvisorConfig | null): string {
	const value = config?.effort;
	if (typeof value === "string") {
		const trimmed = value.trim();
		if ((VALID_EFFORTS as readonly string[]).includes(trimmed)) {
			return trimmed;
		}
	}
	return DEFAULT_EFFORT; // malformed or unknown effort -> default, never a broken flag
}

export function configTimeoutSeconds(config: CodexAdvisorConfig | null): number {
...
export function configReplyBudgetChars(config: CodexAdvisorConfig | null): number {
...
/**
 * Validate and normalize consult arguments (same contract as the Cursor host's
...
export function validateConsult(params: unknown): { values: ConsultValues | null; error: string | null } {
	if (typeof params !== "object" || params === null || Array.isArray(params)) {
		return {
			values: null,
			error:
				"consult_codex_advisor arguments must be an object with task, stage, " +
...
/**
 * The Codex advisor flags, exactly as verified in the Cursor host
 * (`advisor_consult.py:command`): `--ephemeral` keeps the consult out of
 * Codex's session history, `--sandbox read-only` enforces the read-only
 * advisor contract, and the prompt travels on stdin via the trailing `-`.
 * Appended after the resolved CLI's argv prefix.
 */
export function codexAdvisorFlags(model: string, effort: string): readonly string[] {
	return [
		"exec",
		"--ephemeral",
		"--skip-git-repo-check",
 ...
 *   session — a hard failure such as an exhausted Codex quota fails
...
export function denyReason(target: string | null): string {
	...
	return (
		`codex-as-advisor-guardrail: ${what} is denied because the advisor has not been ` +
		`consulted yet this session. This is expected behavior, not an error. Call the ` +
		`consult_codex_advisor tool with task, stage (planning, stuck, pivot-check, or ` +
...
export function unreachableAdvisorMessage(detail: string): string {
	return (
		`Codex advisor is unreachable: ${detail} The write gate is disarmed for this session. ` +
		`Install an authenticated codex CLI, or set harness/${GUARDRAIL_NAME}/config.json ` +
...
export function timeoutAdvisorMessage(detail: string): string {
	return (
		`Codex advisor timed out.${detail ? ` ${detail}` : ""} The write gate is disarmed for ` +
...
export function failedAdvisorMessage(stderr: string, note: string | undefined): string {
	...
	return `Codex advisor failed (transient).${detail} The write gate stays armed: fix the cause and consult again.`;
}
...
export function hardFailureMessage(detail: string): string {
	...
	return (
		`Codex advisor failed and is unusable${why}: ${text} The write gate is disarmed for this ` +
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "consult_codex_advisor",
		label: "Consult Codex Advisor",
		description:
			"Read-only Codex advisor: returns a plan, course correction, or completion " +
			"verdict, pairing every concern with a forward path.",
		promptSnippet:
			"Consult the Codex advisor before the first write, when stuck, before a pivot, and before declaring completion.",
		promptGuidelines: [
			"Call consult_codex_advisor with task, stage, approach, evidence, and question; stage is one of planning, stuck, pivot-check, completion-review.",
		...
		async execute(
			...
			const config = loadHarnessConfig(
				projectRoot,
				GUARDRAIL_NAME,
			) as CodexAdvisorConfig | null;
			if (!isEnabled(config)) {
				throw new Error(
					`codex-as-advisor-guardrail is disabled for this project ` +
			...
			if (!cli.found) {
				...
				throw new Error(unreachableAdvisorMessage(cli.note ?? "codex CLI not found"));
			}

			const argv = [...cli.argvPrefix, ...codexAdvisorFlags(configModel(config), configEffort(config))];
			...
			if (result.status !== "ok") {
				...
				// unusable — authentication, an exhausted Codex quota, a
				// missing model — and must disarm the gate rather than wedge
				// the session; a SOFT failure keeps it armed. Either way the
			...
			if (reply.trim() === "") {
				...
				throw new Error("Codex advisor returned no advice. The write gate stays armed.");
			}
